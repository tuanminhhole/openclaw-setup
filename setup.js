...
docker logs -f openclaw-bot`);
    ...
    if (envInfo) {
      const keys = [];
      if (!provider.isLocal) keys.push(provider.envKey);
      if (ch.envExtra) keys.push(ch.envExtra.split('=')[0]);
      envInfo.textContent = keys.join(' + ') || 'OLLAMA_HOST';
    }
  }
