...
- 🖥️ **"Điều khiển máy" - cho bot mở ứng dụng trên máy bạn**: nút trên thẻ mỗi bot, bật lên là bot mở được mọi ứng dụng đang cài trên máy đang chạy bot (Chrome, TeamViewer, Zalo và các app khác), danh sách đọc thẳng từ Start Menu hoặc thư mục Applications chứ không phải vài cái liệt kê sẵn. Bot cũng chạy được lệnh trên máy đó. **Mặc định TẮT**, cần token, và chỉ dùng trên máy có màn hình.
